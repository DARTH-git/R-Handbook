...
Some of the functions in Rstudio that improve efficiency include the auto-filling option with the use of the 'Tab' button; the automatic closing of brackets, parentheses, and braces; the color-coding of code semantics (e.g., different colors for a numeric varible vs. a string variable vs. a comment); and the automatic identification of errors (e.g. missing closing bracket).
...
```{r, echo = F, fig.cap = "Figure 1: The busy '+' sign in the R Console"}
...
```{r, echo = F, fig.cap = "Figure 2: The RStudio interface"}
...
```{r Rstudio_Editor_ConsoleWin, echo = F, fig.cap= "Figure 3: Writing and transferring commands from the script Editor to the Console"}
...
```{r, echo = T}
# Calculate the density of a normally distributed random variable with mean = 5 and st.deviation = 3 at the value of 5
d_5 <- dnorm(5, mean = 5, sd = 3)

# Calculate the cumulative probability of a value being 5 or lower given that it comes from a normal distribution with mean = 5 and st.deviation = 3
...
```{r}
plot(mydata$age, mydata$sbp, col = 'blue',  ylab = "Systolic Blood Pressure", xlab = "Age", xlim = c(0, 90), ylim = c(80, 200), main = "Blood Pressure vs Age scatter plot.") 
```
...
plot(mydata$age[mydata$gender == "male"], mydata$sbp[mydata$gender == "male"], ylab = 'sbp', xlab= 'age')
points(mydata_new$age[mydata$gender == "female"], mydata$sbp[mydata$gender == "female"], col = 4)


par(mfrow = c(1, 1))
plot(mydata$age[mydata$gender == "male"], mydata$sbp[mydata$gender == "male"], main = "Scatter plot of Age versus SBP for males.", ylab = 'sbp', xlab = 'age')
```
...
## Scatter plot
The plot shown here is a scatterplot of systolic blood pressure at different ages stratified by gender. We first specify the dataset used for plotting, which is mydata_subs1, and define the x- and y-axes (`x = age`, `y = sbp`) in the `aes` object. 
...
hist(res_sbp, freq = F, main = "Histogram and theoretical normal distribution for the residuals.")
